"""
StudyLogix — Friend Manager
Handles friend requests, friend lists, progress tracking, and live timer sharing.
"""

import logging
import sqlite3
import threading

logger = logging.getLogger(__name__)


class FriendManager:
    """Manages friendships and live timer sharing with thread-safe DB access."""

    def __init__(self, db_manager):
        self.db = db_manager
        self.db_lock = threading.Lock()

    def send_friend_request(self, user_id, friend_username):
        """Send a friend request to another user by username."""
        with self.db_lock:
            cursor = self.db.connection.cursor()
            try:
                cursor.execute(
                    "SELECT user_id FROM users WHERE username = ?",
                    (friend_username,),
                )
                result = cursor.fetchone()

                if not result:
                    return False, "User not found."

                friend_id = result[0]

                if user_id == friend_id:
                    return False, "Cannot add yourself as a friend."

                cursor.execute("""
                    SELECT status FROM friendships
                    WHERE (user_id = ? AND friend_id = ?) OR (user_id = ? AND friend_id = ?)
                """, (user_id, friend_id, friend_id, user_id))

                existing = cursor.fetchone()
                if existing:
                    if existing[0] == 'accepted':
                        return False, "Already friends."
                    elif existing[0] == 'pending':
                        return False, "Friend request already pending."
                    elif existing[0] == 'blocked':
                        return False, "Cannot send friend request."

                cursor.execute("""
                    INSERT INTO friendships (user_id, friend_id, status)
                    VALUES (?, ?, 'pending')
                """, (user_id, friend_id))

                self.db.connection.commit()
                return True, "Friend request sent!"

            except sqlite3.Error:
                logger.exception("Error sending friend request")
                return False, "Failed to send friend request."
            finally:
                cursor.close()

    def respond_to_request(self, friendship_id, user_id, accept):
        """Accept or reject a friend request."""
        with self.db_lock:
            cursor = self.db.connection.cursor()
            try:
                cursor.execute("""
                    SELECT user_id, friend_id FROM friendships
                    WHERE friendship_id = ? AND friend_id = ? AND status = 'pending'
                """, (friendship_id, user_id))

                if not cursor.fetchone():
                    return False, "Friend request not found."

                new_status = 'accepted' if accept else 'rejected'
                cursor.execute("""
                    UPDATE friendships
                    SET status = ?, responded_at = CURRENT_TIMESTAMP
                    WHERE friendship_id = ?
                """, (new_status, friendship_id))

                self.db.connection.commit()
                return True, f"Friend request {new_status}."

            except sqlite3.Error:
                logger.exception("Error responding to friend request")
                return False, "Failed to respond to request."
            finally:
                cursor.close()

    def get_pending_requests(self, user_id):
        """Return pending friend requests received by the user."""
        with self.db_lock:
            cursor = self.db.connection.cursor()
            try:
                cursor.execute("""
                    SELECT f.friendship_id, u.username, f.requested_at
                    FROM friendships f
                    JOIN users u ON f.user_id = u.user_id
                    WHERE f.friend_id = ? AND f.status = 'pending'
                    ORDER BY f.requested_at DESC
                """, (user_id,))

                return [
                    {
                        'friendship_id': row[0],
                        'username': row[1],
                        'requested_at': row[2],
                    }
                    for row in cursor.fetchall()
                ]
            except sqlite3.Error:
                logger.exception("Error fetching pending requests")
                return []
            finally:
                cursor.close()

    def get_friends_list(self, user_id):
        """Return the list of accepted friends."""
        with self.db_lock:
            cursor = self.db.connection.cursor()
            try:
                cursor.execute("""
                    SELECT DISTINCT u.user_id, u.username, u.created_at
                    FROM users u
                    JOIN friendships f ON (
                        (f.user_id = ? AND f.friend_id = u.user_id) OR
                        (f.friend_id = ? AND f.user_id = u.user_id)
                    )
                    WHERE f.status = 'accepted'
                    ORDER BY u.username
                """, (user_id, user_id))

                return [
                    {
                        'user_id': row[0],
                        'username': row[1],
                        'member_since': row[2],
                    }
                    for row in cursor.fetchall()
                ]
            except sqlite3.Error:
                logger.exception("Error fetching friends list")
                return []
            finally:
                cursor.close()

    def remove_friend(self, user_id, friend_id):
        """Remove an accepted friendship."""
        with self.db_lock:
            cursor = self.db.connection.cursor()
            try:
                cursor.execute("""
                    DELETE FROM friendships
                    WHERE ((user_id = ? AND friend_id = ?) OR (user_id = ? AND friend_id = ?))
                    AND status = 'accepted'
                """, (user_id, friend_id, friend_id, user_id))

                self.db.connection.commit()
                return True, "Friend removed."

            except sqlite3.Error:
                logger.exception("Error removing friend")
                return False, "Failed to remove friend."
            finally:
                cursor.close()

    def get_friends_progress(self, user_id):
        """Return weekly study progress for all friends."""
        with self.db_lock:
            cursor = self.db.connection.cursor()
            try:
                cursor.execute("""
                    SELECT DISTINCT u.user_id, u.username
                    FROM users u
                    JOIN friendships f ON (
                        (f.user_id = ? AND f.friend_id = u.user_id) OR
                        (f.friend_id = ? AND f.user_id = u.user_id)
                    )
                    WHERE f.status = 'accepted'
                """, (user_id, user_id))

                friends_progress = []
                for row in cursor.fetchall():
                    friend_user_id = row[0]
                    friend_username = row[1]

                    cursor.execute("""
                        SELECT COALESCE(SUM(duration_minutes), 0)
                        FROM study_sessions
                        WHERE user_id = ? AND session_date >= date('now', '-7 days')
                    """, (friend_user_id,))
                    regular_mins = cursor.fetchone()[0]

                    cursor.execute("""
                        SELECT COALESCE(SUM(duration_minutes), 0)
                        FROM pomodoro_sessions
                        WHERE user_id = ? AND status = 'completed'
                        AND DATE(started_at) >= date('now', '-7 days')
                    """, (friend_user_id,))
                    pomodoro_mins = cursor.fetchone()[0]

                    cursor.execute("""
                        SELECT COUNT(*)
                        FROM pomodoro_sessions
                        WHERE user_id = ? AND status = 'completed'
                        AND DATE(started_at) = date('now')
                    """, (friend_user_id,))
                    today_sessions = cursor.fetchone()[0]

                    friends_progress.append({
                        'user_id': friend_user_id,
                        'username': friend_username,
                        'weekly_minutes': regular_mins + pomodoro_mins,
                        'today_sessions': today_sessions,
                    })

                return friends_progress

            except sqlite3.Error:
                logger.exception("Error fetching friends progress")
                return []
            finally:
                cursor.close()

    def get_active_friend_timers(self, user_id):
        """Return currently active timers for all friends."""
        with self.db_lock:
            cursor = self.db.connection.cursor()
            try:
                cursor.execute("""
                    SELECT DISTINCT u.username, t.subject, t.duration_minutes,
                           t.time_remaining, t.is_break, t.last_updated
                    FROM active_timers t
                    JOIN users u ON t.user_id = u.user_id
                    JOIN friendships f ON (
                        (f.user_id = ? AND f.friend_id = t.user_id) OR
                        (f.friend_id = ? AND f.user_id = t.user_id)
                    )
                    WHERE f.status = 'accepted'
                    AND t.time_remaining > 0
                    ORDER BY u.username
                """, (user_id, user_id))

                return [
                    {
                        'username': row[0],
                        'subject': row[1],
                        'duration_minutes': row[2],
                        'time_remaining': row[3],
                        'is_break': bool(row[4]),
                        'last_updated': row[5],
                    }
                    for row in cursor.fetchall()
                ]
            except sqlite3.Error:
                logger.exception("Error fetching active timers")
                return []
            finally:
                cursor.close()

    def update_active_timer(self, user_id, session_id, subject, duration_minutes, time_remaining, is_break):
        """Update or insert the active timer for a user."""
        with self.db_lock:
            cursor = self.db.connection.cursor()
            try:
                cursor.execute("DELETE FROM active_timers WHERE user_id = ?", (user_id,))

                if time_remaining > 0:
                    cursor.execute("""
                        INSERT INTO active_timers
                            (user_id, session_id, subject, duration_minutes,
                             time_remaining, is_break, last_updated)
                        VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
                    """, (user_id, session_id, subject, duration_minutes,
                          time_remaining, int(is_break)))

                self.db.connection.commit()
                return True

            except sqlite3.Error:
                logger.exception("Error updating active timer")
                return False
            finally:
                cursor.close()

    def clear_active_timer(self, user_id):
        """Remove the active timer for a user."""
        with self.db_lock:
            cursor = self.db.connection.cursor()
            try:
                cursor.execute("DELETE FROM active_timers WHERE user_id = ?", (user_id,))
                self.db.connection.commit()
                return True
            except sqlite3.Error:
                logger.exception("Error clearing active timer")
                return False
            finally:
                cursor.close()
